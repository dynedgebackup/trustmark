...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">{{ __('Barangay') }}</h1>

   <a href="#" 
   data-size="lg" 
   data-url="{{ route('barangay.store') }}" 
   data-ajax-popup="true" 
   data-bs-toggle="tooltip" 
   title="{{ __('Manage Barangay') }}" 
...
<div class="row d-flex align-items-center justify-content-end" style="padding-bottom: 20px;">
            <div class="col-md-3">
                <div class="form-group" id="parrent_reg_no">
                    <label for="office_name" class="form-label">{{ __('Municipality | City, Province, Region') }}</label>
                    <span class="validate-err"></span>
                    <div class="form-icon-user">
                        <select name="mun_no_filter" id="mun_no_filter" class="form-control select3" required style="width:100%;">
                            <option value=""></option>
...
$(document).ready(function() {
    select3Ajax("mun_no_filter","parrent_reg_no","getBarngayMunProvRegionAjaxList");
    datatablefunction();
...
function datatablefunction() {
    ...
    $('#dataTable1').DataTable({
        ...
        ajax: {
            url: "{{ route('barangay.getList') }}",
            data: function(d) {
                d.mun_no = $('#mun_no_filter').val();
                d.status = $('#status').val();
                d.q = $('#q').val();
            }
        },
