...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">{{ __('Fees Description') }}</h1>

   <a href="#" 
   data-size="lg" 
   data-url="{{ route('feesDescription.store') }}" 
   data-ajax-popup="true" 
   data-bs-toggle="tooltip" 
   title="{{ __('Manage Fees Description') }}" 
   data-title="Manage Fees Description"
...
function datatablefunction() {
    ...
    $('#dataTable1').DataTable({
        ...
        ajax: {
            url: "{{ route('feesDescription.getList') }}",
            data: function(d) {
                d.q = $('#q').val();
            }
        },
