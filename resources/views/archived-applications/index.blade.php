...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">{{ __('Archived Applications') }}</h1>
                    ...
                    <table id="dataTable1" class="table table-bordered" style="width:100%; table-layout: fixed;">
                        <thead>
                            <tr>
                                ...
                                <th style="width: 120px;">{{ __('Date Archived') }}</th>
...
function datatablefunction() {
    ...
    $('#dataTable1').DataTable({
        ...
        ajax: {
            url: "{{ route('archivedApplicationsReport.getList') }}",
            data: function(d) {
                d.status = $('#status').val();
...
async function loadDataForExcelSheet() {
    ...
    try {
        const exportUrl = `{{ route('archivedApplicationsReport.exportAll') }}?status=${status}&fromdate=${fromdate}&todate=${todate}&q=${q}`;
        ...
        const headerRow = worksheet.addRow([
            ...
            "Business Type", "TIN", "Representative", "Remarks", "Status", "Email Address"
            , "Contact No.", "Date Archived"
        ]);
        ...
        a.href = url;
        a.download = "Archived-Applications.xlsx";
        a.click();
...
window.ActiveInactiveUpdate = function(id, BusinessName, SecurityNo) {
    ...
    swalWithBootstrapButtons.fire({
        title: 'Are you sure?',
        html: "Security No.: <b>" + SecurityNo + "</b><br>" +
              "Business Name: <b>" + BusinessName + "</b><br>" +
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "{{ route('archived-applications.ActiveArchived') }}", 
                type: "POST",
                data: {
                    id: id,
                    is_active: 1,
                ...
                success: function (html) {
                    ...
                    setTimeout(function () {
                        if (typeof datatablefunction === 'function') datatablefunction();
                    }, 1500);
                }
            });
        }
...
$(document).on('click', '.activeinactive', function(e) {
    ...
    const SecurityNo = $(this).data('security');
    if (typeof window.ActiveInactiveUpdate === 'function') {
        window.ActiveInactiveUpdate(id, BusinessName, SecurityNo);
    } else {
        console.error('ActiveInactiveUpdate not available when clicking element.');
    }
});
</script>
