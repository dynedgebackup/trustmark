...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">{{ __('Application Status (Canned Message)') }}</h1>

   <a href="#" 
   data-size="lg" 
   data-url="{{ route('ApplicationStatusCannedMessage.store') }}" 
   data-ajax-popup="true" 
   data-bs-toggle="tooltip" 
   title="{{ __('Manage Application Status (Canned Message)') }}" 
   data-title="Manage Application Status (Canned Message)"
...
<div class="row d-flex align-items-center justify-content-end" style="padding-bottom: 20px;">
            <div class="col-md-3">
                <div class="form-group" id="parrent_reg_no">
                    <label for="app_status_id_filter" class="form-label">{{ __('Application Status') }}</label>
                    <span class="validate-err"></span>
                    <div class="form-icon-user">
                        <select name="app_status_id_filter" id="app_status_id_filter" class="form-control select3" required style="width:100%;">
                            <option value=""></option>
...
$(document).ready(function() {
    select3Ajax("app_status_id_filter","parrent_reg_no","applicationStatusAjaxList");
    datatablefunction();
...
function datatablefunction() {
    ...
    $('#dataTable1').DataTable({
        ...
        ajax: {
            url: "{{ route('ApplicationStatusCannedMessage.getList') }}",
            data: function(d) {
                d.app_status_id = $('#app_status_id_filter').val();
                d.status = $('#status').val();
                d.q = $('#q').val();
            }
        },
