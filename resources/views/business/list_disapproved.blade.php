...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4" style="margin-top: 15px;">
        <h3 class="page-title">Returned Applications</h3>
    </div>
        ...
        <div class="d-flex flex-column" id="content-wrapper">
           ...
           <!--  <div class="row" style="margin-bottom: 15px;">
                    ...
                    <div class="card">
                        ...
                        <div class="row mb-3">
                            <div class="col text-center">
                                <form action="{{ route('business.list-returned') }}" method="GET"
            ...
            <div class="card shadow">
                <div class="card-body">
                    <div class="row">
                         <h6 class="text-primary fw-bold m-0" style="font-size: 12px;">Disapproved Applications</h6>
                    </div>
                     <div class="table-responsive table mt-2" id="dataTable-1" role="grid"
                        aria-describedby="dataTable_info">
                        <table class="table my-0" id="dataTabledisapproved" style="width:100%; table-layout: fixed;">
                            <thead>
                                <tr>
                                    ...
                                    <th class="custom-th" style="width: 150px;">Business Type</th>
                                     <th class="custom-th" style="width: 100px;">Date Disapproved</th>
                                     <!-- <th class="custom-th" style="width: 120px;">Evaluator</th> -->
                                    <th class="custom-th" style="width: 100px;">Remarks</th>
                                    <th class="custom-th" style="width: 50px;text-align:center;">Status</th>
                                ...
                                <!---@forelse ($businesses as $index => $business)
                                    <tr>
                                        ...
                                        <td class="custom-td text-center align-middle">
                                            ...
                                            <span class="badge {{ $badgeClass }} px-2 py-1 small text-center d-inline-block" style="min-width: 80px;">
                                                {{ $status }}
                                            </span>
                                        </td>
    ...
    var DIR =$("#DIR").val();
    const table = $('#dataTabledisapproved').DataTable({
        destroy: true,
        processing: true,
        serverSide: true,
        pagingType: "full_numbers",
        ajax: {
            url: DIR +'get-list-disapproved',
            type: 'GET',
            data: {
                q: $("#details").val(),
        ...
        columns: [
            ...
            render: function(data, type, row, meta) {
                        if (!data) return '';
                        if (data.length > 20) {
                            const shortText = data.substr(0, 20) + '...';
                            return `
                                <div class="text-container" style="white-space: normal; word-break: break-word;">
                                    <span class="short-text">${shortText}</span>
                                    <span class="full-text" style="display:none;">${data}</span>
    ...
    $('#dataTabledisapproved').on('click', '.toggle-text', function(e){
                e.preventDefault();
