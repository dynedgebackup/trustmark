...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4" style="margin-top: 15px;">
        ...
        @if (session('sso_authenticated'))
            <div class="badge bg-success">
                <i class="fas fa-shield-alt"></i> SSO Authenticated
            </div>
        @endif
    </div>
                    ...
                    <div class="card">
                        @if (Auth::user()->role == 1)
                            <div class="card-header">
                                <h6 class="text-primary fw-bold m-0" style="font-size: 12px;">Business
                                    Application
                                </h6>
                            </div>
                        @else
                            <div class="card-header">
                                <h6 class="text-primary fw-bold m-0" style="font-size: 12px;">Business Applications</h6>
                            </div>
                        @endif
                        ...
                        <table class="table my-0" id="dataTable" style="width:100%; table-layout: fixed;">
                            <thead>
                                <tr>
                                    ...
                                    @if (Auth::user()->role == 2)
                                                <th class="custom-th" style="width: 150px;">
                                                    Submitted Date</th>
                                                <!-- <th class="custom-th">
                                                    Date Return</th> -->
                                            <!-- @endif -->
                            ...
                            <div class="table-responsive table mt-2" id="dataTable-1" role="grid"
                                ...
                                <table class="table my-0" id="jqtabledataTable">
                                    <thead>
                                        <tr>
                                            <th class="custom-th">
                                                ID</th>
                                    ...
                                    <tbody>
                                       <!--- @forelse ($businesses as $buss)
                                            <tr>
                                                <td class="custom-td">{{ $loop->iteration }}</td>
                                                <td class="custom-td">
                                                    @php
                                                        $text = $buss->BusinessName ?? 'N/A';
                                                        $previewLimit = 25;
                                                    ...
                                                    @endif</td>
                                                <td class="custom-td" title="{{ $buss->RegistrationNo }}">
                                                    {{ \Illuminate\Support\Str::limit($buss->RegistrationNo, 25, '...') }}
                                                </td>
                                                <td class="custom-td">{{ $buss->TIN }}</td>
                                                <td class="custom-td">{{ $buss->BusinessType ?? ' ' }}</td>
                                                @if (Auth::user()->role == 2)
                                                    <td class="custom-td">{{ $buss->DateSubmitted }}</td>-->
                                                    <!-- <td class="custom-td">{{ $buss->admin_updated_at }}</td> -->
                                                <!--@endif
                                                <td class="custom-td">
                                                    @if ($buss->status == 'APPROVED')
                                                        <span
                                                            class="badge badge-bg-approve p-2 px-3">{{ $buss->status }}</span>
                                                    @elseif ($buss->status == 'UNDER EVALUATION')
                                                        <span
                                                            class="badge badge-bg-evaluation p-2 px-3">{{ $buss->status }}</span>
                                                    @else
                                                        <span
                                                            class="badge badge-bg-returned p-2 px-3">{{ $buss->status }}</span>
                                                    @endif
                                                </td>
                                                <td>
                                                    @if (Auth::user()->role == 1)
                                                        @if ($buss->status == 'UNDER EVALUATION' || $buss->status == 'APPROVED')
                                                            <a href="{{ route('business.view', encrypt($buss->id)) }}"
                                                                data-bs-toggle="tooltip" data-bs-placement="bottom"
                                                                title="View">
                                                                <i class="custom-eye-icon fa fa-eye"></i>
                                                            </a>
                                                        @else
                                                            <a href="{{ route('business.edit', encrypt($buss->id)) }}"
                                                                data-bs-toggle="tooltip" data-bs-placement="bottom"
                                                                title="View">
                                                        ...
                                                        @endif
                                                    @else
                                                        @if ($buss->status == 'UNDER EVALUATION')
                                                            <a href="{{ route('business.view', encrypt($buss->id)) }}"
                                                                data-bs-toggle="tooltip" data-bs-placement="bottom"
                                                                title="View">
                                                                <i class="custom-eye-icon fa fa-eye"></i>
                                        ...
                                        @empty
                                            <tr>
                                                <td class="custom-td text-center"
                                                    colspan="{{ Auth::user()->role == 2 ? 9 : 7 }}">No business found.
                                                </td>
                                            </tr>
                                        @endforelse-->
                                    </tbody>
    ...
    $(document).ready(function() {
        ...
        function datatablefunction() {
            ...
            const table = $('#jqtabledataTable').DataTable({
                ...
                dom: "<'flex justify-start items-center px-4 py-2' l>" + "rt" +
                    "<'flex justify-between items-center px-4 py-2'<'text-sm text-gray-700' i><'flex gap-2' p>>",
                pageLength: 10,
    ...
    @if (session('show_popup'))
        <!-- Bootstrap Modal -->
        <div class="modal fade" id="loginPopupModal" tabindex="-1" aria-labelledby="popupLabel" aria-hidden="true">
