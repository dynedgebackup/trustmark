...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">{{ __('Registered Online Platforms') }}</h1>

   <a href="#" 
   data-size="lg" 
   data-url="{{ route('onlineplatforms.store') }}" 
   data-ajax-popup="true" 
   data-bs-toggle="tooltip" 
   title="{{ __('Manage Online Platform') }}" 
   data-title="Manage Online Platform"
...
<div class="row d-flex align-items-center justify-content-end" style="padding-bottom: 20px;">
            <div class="col-md-4">
                <!-- <div class="form-group" id="parrent_appTypeapp_code_filter">
                    <label for="office_name" class="form-label">{{ __('Application Type') }}</label>
                    <span class="validate-err"></span>
                    <div class="form-icon-user">
                        <select name="app_code_filter" id="app_code_filter" class="form-control select3" required style="width:100%;">
                            <option value=""></option>
...
function datatablefunction() {
   ...
   var table = $('#dataTable1').DataTable({
        ...
        ajax: {
            url: "{{ route('onlineplatforms.getList') }}",
            data: function(d) {
                d.q = $('#q').val();
            }
        },
        ...
        columns: [
         ...
         drawCallback: function(s){ 
            ...
            // If saved page exceeds the total available pages, reset to page 1
...
function ActiveInactiveUpdate(id,is_activeinactive){
    ...
    swalWithBootstrapButtons.fire({
        ...
        {
            $.ajax({
                url: '{{ route("onlineplatforms.ActiveInactive") }}',
                type: "POST", 
                data: {
                    "id": id,
