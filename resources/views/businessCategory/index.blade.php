...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">{{ __('Business Category') }}</h1>

   <a href="#" 
   data-size="lg" 
   data-url="{{ route('businessCategory.store') }}" 
   data-ajax-popup="true" 
   data-bs-toggle="tooltip" 
   title="{{ __('Manage Business Category') }}" 
   data-title="Manage Business Category"
...
function datatablefunction() {
    ...
    $('#dataTable1').DataTable({
        ...
        ajax: {
            url: "{{ route('businessCategory.getList') }}",
            data: function(d) {
                d.q = $('#q').val();
            }
        },
