...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">{{ __('Schedule of Fees') }}</h1>

   <a href="#" 
   data-size="lg" 
   data-url="{{ route('scheduleFees.store') }}" 
   data-ajax-popup="true" 
   data-bs-toggle="tooltip" 
   title="{{ __('Manage Schedule of Fees') }}" 
   data-title="Manage Schedule of Fees"
                        ...
                        <table id="dataTable1" class="table table-bordered" style="width: 98.5% !important;">
                            <thead>
                                <tr>
                                        ...
                                        <th>{{ __('Fee Description') }}</th>
                                        <th>{{ __('Amount') }}</th>
                                        <th>{{ __('Application Fee') }}</th>
                                        <th>{{ __('BMBE Exclusion') }}</th>
                                        <th>{{ __('Status') }}</th>
                                        <th>{{ __('Action') }}</th>
                                    </tr>
                                </thead>
...
function datatablefunction() {
    ...
    $('#dataTable1').DataTable({
        ...
        ajax: {
            url: "{{ route('scheduleFees.getList') }}",
            data: function(d) {
                d.app_code = $('#app_code_filter').val();
                d.status = $('#status').val();
                d.q = $('#q').val();
            }
        },
        ...
        columns: [
            ...
            { data: 'amount' },
            { data: 'is_application_fee' },
            { data: 'exclude_due_to_bmbe' },
            { data: 'status' },
